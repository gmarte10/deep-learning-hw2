...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        ta_avg = 0
        for item in metrics["train_acc"]:
            ta_avg += torch.mean(item)
        ta_avg = ta_avg / len(metrics["train_acc"])

        logger.add_scalar("train_accuracy", ta_avg, global_step)
        ...
        for _ in range(10):
            ...
            # TODO: save additional metrics to be averaged
            metrics["val_acc"].append(torch.mean(dummy_validation_accuracy))

        # TODO: log average val_accuracy
        va_avg = sum(metrics["val_acc"]) / len(metrics["val_acc"])
        logger.add_scalar("val_accuracy", va_avg, global_step)
